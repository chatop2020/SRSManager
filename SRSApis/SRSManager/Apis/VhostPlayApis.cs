...
namespace SRSApis.SRSManager.Apis
{
    public static class VhostPlayApis
    {
        ...
        /// <param name="rs"></param>
        /// <returns></returns>
        public static bool DeleteVhostPlay(SrsManager sm, string vhostDomain, out ResponseStruct rs)
        {
        ...
        /// <param name="rs"></param>
        /// <returns></returns>
        public static Play GetVhostPlay(SrsManager sm, string vhostDomain, out ResponseStruct rs)
        {
            if (sm == null || sm.Srs == null || sm.Srs.Vhosts == null)
            {
                rs = new ResponseStruct()
        ...
        /// <returns></returns>
        public static bool SetVhostPlay(SrsManager sm, string vhostDomian, Play play,
            out ResponseStruct rs, bool createIfNotFound = false)
        {
            ...
            if (found)
            {
                rs = new ResponseStruct()
                {
                    Code = ErrorNumber.None,
                    Message = rs.Message += "\r\n" + "play更新成功" +
                                            JsonHelper.ToJson(play),
                };
                //找到了
                sm.Srs.Vhosts[i].Vplay = Common.ObjectClone(play);
                return true;
            }
            else
            {
                ...
                {
                    ret = CreateVhostPlay(sm, vhostDomian, play, out rs);
                    if (ret)
                    {
                        rs = new ResponseStruct()
                        {
                            Code = ErrorNumber.None,
                            Message = rs.Message += "\r\n" + "SRS实例中未有指定Play内容，系统已经将传入Play创建到SRS实例中" +
                                                    JsonHelper.ToJson(play),
                        };
                        return true;
                    }
        ...
        /// <param name="rs"></param>
        /// <returns></returns>
        public static bool CreateVhostPlay(SrsManager sm, string vhostDomain, Play play,
